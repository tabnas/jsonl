...
#
# Only this crate's entry is asserted. The siblings' entries legitimately
# move whenever their checkouts do, which is the same reason blanket
...
# exemption: the recorded version of each sibling crate. Those entries
# legitimately move whenever the sibling checkouts do (the engine, the
# strict-JSON grammar and the fixture runner are all resolved from a
# sibling's MAIN), and exempting exactly them is what makes a full
# comparison usable here when blanket `--locked` is not.
lock_without_sibling_versions() {
  awk '
    /^\[\[package\]\]$/                                   { sib = 0 }
    /^name = "(tabnas|tabnas-json|tabnas-support)"$/     { sib = 1 }
...
#
# Cargo.lock records each sibling by version, and the siblings are resolved
# from checkouts of MAIN. So the day parser, json or support bumps its
# crate version, `--locked` here fails with "cannot update the lock file"
# on every pull request in this repo, including ones that touch no Rust at
# all -- a red build caused by another repository's release. The engine
# repo has no path dependency of its own, which is why `--locked` is right
# there and wrong here.
#
# NOT `--all` on fmt. cargo defines it as "all packages, and also their
# local path-based dependencies", and the siblings ARE such dependencies,
# so `--all` reaches into their checkouts: an unformatted file over there
# fails this gate even when every file here is clean.
"${CARGO[@]}" fmt --check
