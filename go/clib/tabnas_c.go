...
// Package main builds the C-ABI shared library: libtabnasjsonl.
//
// tabnas-clib-template: v1 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnasjsonl.so ./clib
//
// jsonl TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a jsonl parser and returns a handle to it.
...
// tabnas_parse checks one input against the jsonl grammar.
//
//export tabnas_parse
